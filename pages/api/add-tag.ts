...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    console.log('Attempting to add tag:', { originalPublicId: public_id, fullPublicId, tag });

    try {
      // Add the tag using the uploader API
      const result = await cloudinary.uploader.add_tag(tag, [fullPublicId], {
        resource_type: 'image'
      });
